import pandas as pd
from tqdm import tqdm

sales = pd.read_pickle("data/prepared/sales.pkl")
stores = pd.read_pickle("data/prepared/stores.pkl").rename(columns={"id": "store"})

sales = pd.merge(left=sales, right=stores, on="store", how="left")

sales["day_of_week"] = sales.date.dt.day_of_week

aggs = {
    ("item", "vocation"): ["mean"],
    ("item", "region"): ["mean"],
    ("item", "size"): ["mean"],
    ("item", "surface"): ["mean"],
    ("item", "vocation", "day_of_week"): ["mean"],
    ("item", "region", "day_of_week"): ["mean"],
    ("item", "size", "day_of_week"): ["mean"],
    ("item", "surface", "day_of_week"): ["mean"],
}

for by, hows in tqdm(aggs.items()):

    for how in hows:

        how_name = how.__name__ if callable(how) else how
        name = f"{how_name}_" + "_x_".join(by)
        path = f"data/features/{name}.pkl"
        agg = sales.groupby(list(by))["quantity"].agg(how)
        agg = agg.rename(name)
        agg.to_pickle(path)
